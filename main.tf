locals {
  ...
  enable_ecs_service_role = module.this.enabled && var.ecs_service_role_enabled && var.network_mode != "awsvpc" && length(var.ecs_load_balancers) <= 1
...
resource "aws_ecs_task_definition" "default" {
  ...
  container_definitions    = nonsensitive(var.container_definition_json)
